...
GalleryCandidate.prototype.init = function () {
    ...
    $("#input_big, #input_small").livequery('keydown keyup', function (e) {  //按下 Enter 隱藏
        ...
        if (e.keyCode == 13) self.candidateDiv.fadeOut('fast', function () {
            $(this).remove()
        });
    });
    ...
    setInterval(function () {
        if (!self.enabled) return;
        if ($("#input_big").length && $("#input_big").val().length == 0) $("#main_poster .candidate").fadeOut('fast', function () {
            $(this).remove()
        });
        if ($("#input_small").length && $("#input_small").val().length == 0) $(".mini_form .candidate").fadeOut('fast', function () {
            $(this).remove()
        });
    }, 200);
    ...
    _.each(emotions, function (emoticon) {
        ...
        td.bind("click", function (e) {
            ...
            if (((!isMac && e.ctrlKey) || isMac && e.metaKey) && e.shiftKey) { //ctrlKey delete
                ...
                self.candidateDiv.fadeOut('fast', function () {
                    $(this).remove()
                });
                if (isCustom) {
...
GalleryCandidate.prototype.show = function (parent) {
    ...
    if (!input) return false; // not ready
            ...
            for (var i in emotions) {
                ...
                for (var k in keywords) { // for all keyword patterns
                    try {
                        ...
                        if (kw.match(/^\s*$/) || kw == undefined) continue; // skip empty keyword match
...
CandidatePlugin.prototype.stop = function () {
    ...
    $("#main_poster .candidate, .mini_form .candidate").fadeOut('fast', function () {
        $(this).remove()
    });
};
