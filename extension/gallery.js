...
Gallery.prototype.init = function () {
    ...
    $(".cmp_emoticon_on, .cmp_emoticon_off, #main_poster .emoticon-selector_img_on, #main_poster .emoticon-selector_img_off").livequery("click", function () {
        self.lastInputFocused = document.getElementById('input_big');
    });
    $(".cmp_emoticon_mini_on, .cmp_emoticon_mini_off, .mini_form .emoticon-selector_img_on, .mini_form .emoticon-selector_img_off").livequery("click", function () {
        self.lastInputFocused = document.getElementById('input_small');
    });
...
Gallery.prototype.getPanel = function () {
    ...
    if (self.emoticonsPanel) {
        ...
        //var panel = self.emoticonsPanel = $($('#emoticons-holder').get(0) || $('<div id="emoticons-holder" />').css('height', '250px').prependTo('#emoticon-selector-holder')).show().children().hide().end();
        var panel = self.emoticonsPanel = $($('#emoticons_show').get(0) || $('<div id="emoticons_show" />').css('height', '250px').prependTo('#emoticon-selector-holder')).show().children().hide().end();

        var loading = self.loading = self.loading || $('<div/>', {
                class: 'loading',
                text: 'Loading...'
            }).appendTo(panel).show();
        ...
        $('#emoticon-tabs li[id^=emo_], #emoticon-tabs li.emo_my_tab ').click(function () {
            $('#emoticon-selector #emoticon_holder_super_parent').show();
            $('#emoticons_show').hide();
        });
...
Gallery.prototype.registerTab = function (className, opener, label) {
    ...
    $("#emoticon-tabs ul:first").livequery(function () {
        ...
        self.tabs[className] = $('<li>', {
            ...
            click: function (e) {
                ...
                $('#emoticon-selector').toggleClass('pc-gallery', true);
        ...
        self.storage.online.getStorageLimit(function (limit) {
            ...
            if (removable.length + self.emotionUsed.length > limit - 5) {
                ...
                self.storage.online.removeEmotion(toRemove.url, function () {
                    uploadAndUse(keyword, url, function () {
...
Gallery.prototype.open = function (className, inputTarget) {
    if($("#emoticon-selector-holder").is(":visible")){
...
Gallery.prototype.openGallery = function () {
    var self = this;
    $('#emoticon-tabs').find('li.gallery').livequery(function () {
...
Gallery.prototype.showGallerySeen = function (wrapper, loading) {
    ...
    this.storage.loadEmotions(function (localEmoticons) {
        ...
        emoticons = _.reject(emoticons, function (e) {
            return _.findWhere(localEmoticons, {hash_id: e.hash_id});
        });

        emoticons = _.sortBy(emoticons, function (e) {
            return (now - e.time) / e.count;
        });
...
Gallery.prototype.generateGalleryTab = function () {
    ...
    $("<a class='switchWindowMode' style='float:right;height:9px;width:9px; margin:8px; padding:2px; background:url(http://emos.plurk.com/633e54d3723da4e8c1acc48288e952bc_w8_h8.gif) no-repeat; cursor:pointer;' title='切換視窗大小'></a>")
        .on('click', function () {
            $('body').toggleClass('large_emoticon-selector');
            $('#emoticon-selector').toggleClass('large');
            $('.tableWrapper').scrollTop(0);
            self.lazyLoaders[self.currentClassName] && self.lazyLoaders[self.currentClassName].start();
        }).insertAfter("#emoticon-tabs #emo_emotiland_button");
    ...
    //Make it draggable
    // $("#popViewContent").draggable({attachTo: "#emoticon-tabs", ignore: "a, li", "cursor": "move" })
    // createStyle(doc, "#emoticon-selector.ondrag {opacity: 0.5;}");
    // createStyle(doc, "#emoticon-selector {-webkit-box-shadow: rgba(0, 0, 0, 0.8) 2px 2px 5px 0px;-webkit-transition: opacity 0.2s linear;}");
    //createStyle(doc, "#emoticon-selector #emoticon-tabs {cursor: move ;}");
};
    ...
    var filter = $("<input class='filter' type='text' placeholder='" + __('快速找到相關的表情') + "' />")
        .keyup(function () {
            ...
            delayTimer = setTimeout(function () {
                ...
                //hide others
                var animated = tds.not(matched).filter(':visible').filter(':lt(25)').css({
                    'max-width': '0',
                    'max-height': '0',
                    'opacity': '0',
                    'display': ''
                ...
                tds.not(matched).not(":visible").filter(':lt(25)').css({
                    'opacity': '1',
                    'display': '',
                    'max-width': '0',
                    'max-height': '0'
...
AdvancedGalleryPlugin.prototype.start = function () {
    ...
    $('#emo_my, #emo_extra, #emo_basic').livequery(function () {
        $('#emoticon-tabs').find('.gallery.default, .gallery.online').show();
        $(this).hide();
    });
    $('.emoticon-selector_img_off').livequery('click', function () {
        setTimeout(function () {
...
AdvancedGalleryPlugin.prototype.stop = function () {
    $('#emo_my, #emo_extra, #emo_basic').livequery(function () {
        $('#emoticon-tabs').find('.gallery.default, .gallery.online').hide();
        $(this).show();
    });
    $('.emoticon-selector_img_off').expire('click');
};
